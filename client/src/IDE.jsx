...
const saveCodeToServer = async (code) => {
  try {
    await axios.post(`${API_BASE_URL}/api/save`, { code });
  } catch (error) {
    console.error('Failed to save code:', error);
  }
};

const loadCodeFromServer = async () => {
  try {
    const res = await axios.get(`${API_BASE_URL}/api/load`);
...
clap: 'data:audio/wav;base64,UklGRm4BAABXQVZFZm10IBAAAAABAAEARKwAAIhYAQACABAAZGF0YUoBAAC4uLi4uLi4uLi4uLi4uLi4uLi4uLi4uLi4uLi4uLi4uLi4uLi4uLi4uLi4uLi4uLi4uLi4uLi4uLi4uLi4uLi4uLi4uLi4uLi4uLi4uLi4uLi4uLi4uLi4uLi4uLi4uLi4uLi4uLi4uLi4uLi4uLi4uLi4uLi4uLi4uLi4uLi4uLi4uLi4uLi4uLi4uLi4'
};
...
return (
<div className={`min-h-screen bg-gray-900 text-gray-100 font-mono ${uiCorruption > 0.5 ? 'transform rotate-1' : ''}`}>
{/* Header */}
<div className={`bg-gray-800 border-b border-gray-700 px-4 py-2 flex items-center justify-between ${uiCorruption > 0.3 ? 'animate-pulse' : ''}`}>
<div className="flex items-center space-x-4">
<h1 className={`text-xl font-bold text-blue-400 ${uiCorruption > 0.7 ? 'font-serif' : ''}`}>
404 IDE - Developer Environment v2.404.1
</h1>
<div className="flex items-center space-x-2">
<div className="w-3 h-3 bg-red-500 rounded-full animate-pulse"></div>
<span className="text-xs text-gray-400">Status: Barely Functional</span>
</div>
</div>
...
<span className="font-semibold">Project: Disaster</span>
</div>
...
file.name === activeFile ? 'bg-gray-700 text-blue-400' : 'text-gray-300'
}`}
...
{/* Metrics Panel */}
{showMetrics && (
<div className={`p-4 border-t border-gray-700 ${uiCorruption > 0.6 ? 'transform -rotate-2' : ''}`}>
<h3 className="font-semibold mb-3 text-yellow-400">📊 Development Metrics</h3>
<div className="space-y-2 text-xs">
<div className="flex justify-between">
<span>Code Confidence:</span>
<span className="text-red-400">{confidence}%</span>
</div>
<div className="flex justify-between">
<span>Editor Self-esteem:</span>
<span className="text-red-400">{selfEsteem.toFixed(1)}</span>
</div>
<div className="flex justify-between">
<span>Chaos Level:</span>
<span className="text-orange-400">{chaosLevel.toFixed(1)}/10</span>
</div>
<div className="flex justify-between">
...
<span className="text-red-400">{Math.floor(timeWasted / 10)}</span>
</div>
<div className="flex justify-between">
<span>Hope Remaining:</span>
<span className="text-purple-400">{Math.max(0, 100 - timeWasted)}%</span>
</div>
</div>
</div>
)}
...
{/* Toolbar */}
<div className={`bg-gray-800 border-b border-gray-700 px-4 py-2 flex items-center space-x-4 ${uiCorruption > 0.4 ? 'animate-bounce' : ''}`}>
<button
onClick={handleRun}
disabled={isRunning}
...
<button className="flex items-center space-x-2 px-4 py-2 bg-blue-600 hover:bg-blue-700 rounded transition-colors">
<Save className="w-4 h-4" />
<span>Save (Pretend)</span>
</button>

<button className="flex items-center space-x-2 px-4 py-2 bg-gray-600 hover:bg-gray-700 rounded transition-colors">
...
className="flex items-center space-x-2 px-4 py-2 bg-gray-600 hover:bg-gray-700 rounded transition-colors"
>
<Terminal className="w-4 h-4" />
<span>Console</span>
{errors.length > 0 && (
<span className="bg-red-500 text-white text-xs px-2 py-1 rounded-full">{errors.length}</span>
)}
</button>
</div>
...
<div
key={file.name}
className={`px-4 py-2 bg-gray-700 rounded-t-lg text-sm flex items-center space-x-2 ${
...
<span>{file.name}</span>
<X
className="w-3 h-3 hover:bg-gray-600 rounded cursor-pointer"
onClick={() => {
...
<div
key={file.name}
className="px-3 py-2 bg-gray-600 rounded-t-lg text-sm flex items-center space-x-2 opacity-60 cursor-pointer hover:opacity-80 transition-opacity"
onClick={() => handleFileSwitch(file.name)}
...
className={`w-full h-full bg-gray-900 text-gray-100 font-mono text-sm p-4 resize-none outline-none ${
...
<div className="absolute top-20 left-8 bg-gray-800 border border-gray-600 rounded shadow-lg p-2 z-10">
<div className="text-xs text-gray-400 mb-2">Suggestions (Probably Wrong):</div>
{TROLL_COMPLETIONS.slice(0, 5).map((completion, i) => (
<div
key={i}
className="px-2 py-1 hover:bg-gray-700 cursor-pointer text-sm text-yellow-400"
>
{completion}
</div>
))}
...
{/* Console */}
{showConsole && (
<div className={`bg-black border-t border-gray-700 p-4 h-64 overflow-y-auto ${uiCorruption > 0.6 ? 'animate-pulse' : ''}`}>
...
{line && <span className="text-gray-500">{'>'} </span>}
{line}
</div>
))}
{errors.length > 0 && (
...
{showTooltip.show && (
<div
className="fixed bg-gray-800 border border-gray-600 rounded px-2 py-1 text-xs z-50 pointer-events-none"
...
<div className="bg-gray-800 border border-gray-600 rounded-lg p-6 max-w-md mx-4 animate-bounce">
<div className="flex items-center space-x-3 mb-4">
<AlertTriangle className="w-6 h-6 text-yellow-400" />
<h3 className="font-semibold text-lg">IDE Notification</h3>
</div>
...
<h1 className="text-4xl font-bold mb-4">CRITICAL STOP</h1>
<p className="text-xl mb-2">404 IDE has encountered a problem and needs to restart.</p>
<p className="text-lg mb-4">Your code was so bad it broke the IDE.</p>
<p className="text-sm">Error Code: 0x0000DEAD</p>
<p className="text-sm">Collecting error info: {Math.floor(Math.random() * 100)}%</p>
...
<div className="flex items-center space-x-4">
<span className="text-gray-400">Line 1, Column 1</span>
<span className="text-gray-400">UTF-8</span>
<span className="text-gray-400">JavaScript (Broken)</span>
</div>
<div className="flex items-center space-x-4">
<div className="flex items-center space-x-2">
<Bug className="w-4 h-4 text-red-400" />
<span className="text-red-400">{errors.length} Problems</span>
</div>
<div className="flex items-center space-x-2">
<Zap className="w-4 h-4 text-yellow-400" />
<span className="text-yellow-400">IntelliSense: Confused</span>
</div>
</div>
</div>
</div>
